<?php
global $wpdb;
$specification=$wpdb->get_results($wpdb->prepare("SELECT *FROM {$wpdb->prefix}bd_specification WHERE ID=%d",$_GET['showreadyhomesdetail']));
...
<?php $planimages=(array)json_decode($specification->SpecImages);
	...
	<?php else:
		if($planimages['ElevationImage']!=''):
	?>
	
	<div style="width:98%;margin:auto;">
	 <img class="shadow" style="width:100%;" src="<?php echo $planimages['ElevationImage'];?>"  alt="<?php echo $eleimage;?>" />
	</div>
	<?php endif;endif; ?>     
    ...
    <div class="">
    	<div class="span4" style="border-left: 0;">
            <div class="detail_left_content_module" >
				<div>
				<h2 class="spec_head">
				<?php $pname=$wpdb->get_results($wpdb->prepare("SELECT *FROM {$wpdb->prefix}bd_plan WHERE ID=%d",$specification->plan_id));
				echo $pname[0]->plan_name ;;?>
  ...
  <tr>
    ...
    <td><?php $ftype=(array)json_decode($specification->Attributes); echo $ftype['Type'];?></td>
			   ...
			   <?php if(count($divisions)>0):?>
				...
				<h2 class="spec_head">Available To Build</h2>
				<ul>
	...
	<div class="span8">
       ...
       <div class="floorplan2"><h2>
		...
		<?php $planimages=(array)json_decode($specification->SpecImages);
		if(!is_array($planimages['FloorPlanImage'])&&$planimages['FloorPlanImage']&&checkRemoteFile($planimages['FloorPlanImage'])):
		?>
		<div class="floorplan">
		...
		$images=$planimages['InteriorImage'];
		echo '<h3>Additional Images</h3>';
		if(is_array($images)||is_object($images)):
		foreach($images as $key=>$pimage):
		?>
		<?php if(is_array($pimage)||is_object($pimage)):?>
			...
			<a rel="prettyPhoto[gallery]" href="<?php echo $pimgurl;?>">
				<img class="shadow"  width="125" height="94" src="<?php echo $dimageurl;?>">
				</a>
		...
		<?php endforeach;?>
		<?php else:?>
		<a rel="prettyPhoto[gallery]" href="<?php echo $images;?>">
		<img class="shadow"  width="125" height="94" src="<?php echo $images;?>">
		</a>
		<?php endif;?>
		...
		<h2 class="spec_head">Virtual Tour / Video</h2>
