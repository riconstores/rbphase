...
$noofitems= $wpdb->get_results($wpdb->prepare("SELECT *FROM {$wpdb->prefix}bd_specification  HAVING SpecIsModel=%d",1));
$limit=30;
...
$specifications= $wpdb->get_results($wpdb->prepare("SELECT *FROM {$wpdb->prefix}bd_specification  HAVING SpecIsModel=%d LIMIT $start, $limit",1));
...
<?php $i=0; $beaches;
		...
		{ $address=json_decode($specification->SpecAddress);
			...
			$beaches['m'.$i]['url']='http://'.$_SERVER["HTTP_HOST"].$rurl[0].'&model_detail='.$specification->ID;
...
'</h4><span><strong>'+beaches[key].title+'</strong><br><span style="font-size: 11px;">'+
	...
	'</span>'+
	'<ul style="margin: 0px; width: 127px;" class="callout_nav01">'+
...
<div class="plans">
 ...
 <section class="full_width_cont">
    ...
    <div class="divisions">
		...
		{
		Echo '<h1 style=Font-size:20px;>There are no data available</h1>';
		}
		?>
		<div class="collection_sort_row02">
				...
				<div class="hor_mod_wrapper">
					...
					<a href="http://<?php echo $_SERVER["HTTP_HOST"].$rurl[0] ;?>&model_detail=<?php echo $specification->ID;?>">
